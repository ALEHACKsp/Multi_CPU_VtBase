...
ULONG NTAPI VmxAdjustControls(ULONG Ctl,ULONG Msr)
{
	LARGE_INTEGER MsrValue;

	MsrValue.QuadPart = Asm_Rdmsr(Msr);
	Ctl &= MsrValue.HighPart;     /* bit == 0 in high word ==> must be zero */
	Ctl |= MsrValue.LowPart;      /* bit == 1 in low word  ==> must be one  */
	return Ctl;
}

NTSTATUS FillGuestSelectorData(ULONG GdtBase, ULONG Segreg, USHORT Selector)
{
	...
	Vmx_VmWrite(GUEST_ES_BASE + Segreg * 2, (ULONG)SegmentSelector.base);
